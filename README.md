...
Top-right of row 1 shows the current temperature and a small precipitation glyph — a droplet for rain, a snowflake for snow — when precipitation is at least 40% likely in the current hour or the next two. That look-ahead reaches 2-3 hours out (the current hour is already partly elapsed), so the sign answers "do I need an umbrella" rather than "is it raining this instant". Data comes from the National Weather Service (`api.weather.gov`) — no API key, US-only, 10-minute server-side cache.
